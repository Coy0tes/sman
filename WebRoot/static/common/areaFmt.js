...
$(function(){
	//将手机端区域格式(larea.js)转化成PC端需要的区域格式(jsaddress.js)
	var joinlist = "";
	$.each(lAreaData,function(i,v){
		var eachp = "{name:'" + v.name.replace(/(^\s*)|(\s*$)/g, "") + "',cityList:[";
		
		var joincitylist = "";
		$.each(v.child,function(ii,vv){
			var eachcity = "{name:'" + vv.name.replace(/(^\s*)|(\s*$)/g, "") +"',areaList:[";
			
			if(vv.child != undefined){
				var countylist = "";
				$.each(vv.child,function(iii,vvv){
					countylist += "'"+vvv.name.replace(/(^\s*)|(\s*$)/g, "")+"',";
				});
				eachcity += countylist.substring(0,countylist.length-1);
			}
			eachcity += "]},";
			
			joincitylist += eachcity;
		});
		
		eachp += joincitylist.substring(0,joincitylist.length-1) + "]},";
		
		joinlist += eachp;
	});
	
	joinlist = joinlist.substring(0,joinlist.length-1);
	
	document.write("var provinceList = [" + joinlist + "];");
});