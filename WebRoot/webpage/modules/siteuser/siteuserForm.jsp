	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			validateForm = $("#inputForm").validate({
				rules: {
					loginName: {remote: "${ctx}/sys/user/checkLoginName?oldLoginName=" + encodeURIComponent('${siteuser.loginName}')}//设置了远程验证，在初始化时必须预先调用一次。
				},
				messages: {
...
<body class="hideScroll">
		<form:form id="inputForm" modelAttribute="siteuser" action="${ctx}/siteuser/siteuser/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<sys:message content="${message}"/>	
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
				<tr>
					<td class="width-15 active"><label class="pull-right"><font color="red">*</font>场馆：</label></td>
					<td class="width-35">
						<sys:gridselect url="${ctx}/siteuser/siteuser/selectsite" id="site" name="site.id"  value="${siteuser.site.id}"  title="选择场馆" labelName="site.name" 
				...
				<tr>
					...
					<td class="width-35">
						<c:if test="${empty siteuser.id}">
							<form:input path="loginName" htmlEscape="false"    class="form-control required"/>
						</c:if>
						<c:if test="${not empty siteuser.id}">
							<form:input path="loginName" htmlEscape="false"    class="form-control required" readonly="true"/>
						</c:if>
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right"><c:if test="${empty siteuser.id}"><font color="red">*</font></c:if>密码：</label></td>
					<td class="width-35">
					    <input id="newPassword" name="newPassword" type="password" value="" maxlength="50" minlength="3" class="form-control ${empty siteuser.id?'required':''}"/>
						<c:if test="${not empty siteuser.id}"><span class="help-inline">若不修改密码，请留空。</span></c:if>
					</td>
				</tr>
				<tr>
					<td class="active"><label class="pull-right"><c:if test="${empty siteuser.id}"><font color="red">*</font></c:if>确认密码:</label></td>
		            <td><input id="confirmNewPassword" name="confirmNewPassword" type="password"  class="form-control ${empty siteuser.id?'required':''}" value="" maxlength="50" minlength="3" equalTo="#newPassword"/></td>
				</tr>
				<tr>
