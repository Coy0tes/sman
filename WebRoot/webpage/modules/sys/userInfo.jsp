									...
									<div class="table-responsive">
										<table class="table table-bordered">
											<tbody>
												...
												<c:if test="${agencyFlag == true}">
													<tr>
														<td><strong>所在区域</strong></td>
														<td>${agency.province} - ${agency.city } - ${agency.county }</td>
													</tr>
													<c:forEach items="${agency.agencyAreaList }" var="area" varStatus="status">
														<tr>
														   <td><strong>代理区域-${status.index + 1 }</strong></td>
														   <td>${area.province} - ${area.city } - ${area.county }</td>
														</tr>
													</c:forEach>
												</c:if>
												<%-- <tr>
										...
										<strong>上次登录</strong> 时间：<fmt:formatDate value="${user.oldLoginDate}" type="both" dateStyle="full"/>
